...
import { buildGunmaMap } from './gunma-map.js?v=20260925-cloudsea-3';
import { createMountainAtmosphere, createCanopyShade } from './gunma-atmosphere.js?v=20260925-cloudsea-3';
...
(function () {
  ...
  // 明るい順に並べ、間は直線で補間する。30%までは昼のまま、そこから
  // 暗くなり始めて17%で半分、15%を過ぎると一気に落ち、10%以下は
  // 夜間モードと同じ暗さで頭打ち。
  const DUSK_BY_METER = [
    [0.30, 0], [0.29, 0.1], [0.25, 0.2], [0.225, 0.3],
    [0.20, 0.4], [0.17, 0.5], [0.15, 0.75], [0.10, 1], [0, 1],
  ];
  ...
  // ループ地点をまたいだCPU車は、ユーザー車から見て一番近いコピーの位置へ描く。
  // ずらす量は必ずコピー間隔の整数倍にすること。半端にずらすとコピーの道路から
  // 横へ外れ、壁を突き抜けたり空中を走ったりする。
  // 継ぎ目では車の描画位置が数十m跳ねるが、これは地図側の都合で、走行ラインの
  // 端から端(約3277m)とコピーの間隔(約3320m)が一致していないため。
  // 跳ねを消そうと整数倍でないずらし方をすると、上の「道路から外れる」を招く。
  function updateLoopDrawZ(ai) {
    if (!(car2VisualLoopSpanZ > 0)) {
      ai.drawRelZ = ai.pos.z - player.pos.z;
      return ai.pos.z;
    }
    const copies = Math.round((player.pos.z - ai.pos.z) / car2VisualLoopSpanZ);
    const z = ai.pos.z + copies * car2VisualLoopSpanZ;
    ai.drawRelZ = z - player.pos.z;
    return z;
  }
  ...
  async function init() {
    ...
    if (MAP_GLTF) {
      ...
      if (COURSE_KEY === 'gunma') {
        ...
        gunmaAtmosphere = createMountainAtmosphere(scene, valleyFloor + gunmaCourse.climb * 0.38, gunmaCourse.route,
  ...
  function updateAI(dt, sigStates) {
    ...
    for (const ai of aiCars) {
      ...
      ai.group.position.copy(ai.pos);
      ai.group.position.z = updateLoopDrawZ(ai);
      ai.group.rotation.y = ai.heading;
