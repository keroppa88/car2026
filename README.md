...
* 群馬はThree.jsで幅7.2mの2車線山道を生成。直線、緩い左右カーブ、左右のヘアピンをつなぎ、白いガードレールと両側の森を配置する。1周の前半で上り、後半で下る（通常走行で各約2分）。ガードレールへの接触では進行方向の速さを保つ。読み込みごとに形が変わり、`?course=gunma&seed=1234` で再現できる。
