...
* ぐんまーはThree.jsで幅8.64mの2車線山道を生成。直線、緩い左右カーブ、左右のヘアピンをつなぎ、両側に白いガードレールと芝生を配置する。木は置かない。1周の前半で上り、後半で下る（通常走行で各約2分）。標高差は従来の1.65倍。ガードレールへの接触では進行方向の速さを保つ。読み込みごとに形が変わり、`?course=gunma&seed=1234` で再現できる。
