...
(function () {
  ...
  const DEMO_SEQUENCE_STAGE_MS = 15000;
  ...
  function updateWeatherEffects(dt) {
    ...
    if (weatherStarSystem) {
      weatherStarSystem.visible = weatherStars;
      weatherStarSystem.position.copy(camera.position);
    }
    if (weatherSmallStarSystem) {
      weatherSmallStarSystem.visible = weatherStars;
  ...
  function applyNight() {
    applyWeatherSky();
    hemi.intensity = nightMode ? 0.13 : 0.55;
    hemi.color.set(nightMode ? 0x707e9e : 0xffffff);
    hemi.groundColor.set(nightMode ? 0x14161c : 0x6f7168);
    sun.intensity = nightMode ? 0.04 : 0.85;
    sun.color.set(nightMode ? 0xbfd0ff : 0xffffff);
    refreshLampVisibility();
    // 夜間は自車を toyota86n.vox(ライト拡大版)へ切り替える。
    // 地図側の発光を許可したコースだけ、白面を夜間発光させる。
    for (const glowMesh of mapWhiteGlowMeshes) glowMesh.visible = nightMode;
    document.body.dataset.mapWhiteGlowVisible = nightMode
    ...
    for (const roadMesh of car2RoadMeshes) {
      for (const mat of (Array.isArray(roadMesh.material) ? roadMesh.material : [roadMesh.material])) {
        ...
        if (nightMode && MAP_CONFIG.roadNightEmissive !== false) {
  ...
  const INDY_CORNER_DRIFT_KMH = 152;
